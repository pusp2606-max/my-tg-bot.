...
async def bio_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "Отлично! Пришли фото для профиля."
    )
...
async def photo_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "Фото сохранено! Напиши свой регион:"
    )
...
async def region_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "Теперь напиши свои интересы через запятую:"
    )
...
async def interests_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    interests = ",".join(
        [i.strip().lower() for i in update.message.text.split(",")]
    )
    ...
    await update.message.reply_text(
        "Профиль создан! Используй 🔎 Поиск для поиска других пользователей.",
        reply_markup=main_menu()
    )
...
async def browse(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with aiosqlite.connect("bot.db") as db:
        ...
        async with db.execute(
            "SELECT region, interests FROM users WHERE id=?",
            (user_id,)
        ) as cursor:
        ...
        if not me:
            await update.message.reply_text(
                "Сначала создай профиль через /setprofile"
            )
            return
        ...
        my_interests_set = (
            set(my_interests.split(","))
            if my_interests else set()
        )

        async with db.execute(
            """
            SELECT id, name, bio, photo, interests
            FROM users
            WHERE id != ? AND region=?
            """,
            (user_id, region)
    ...
    await update.message.reply_text(
        "Нет доступных пользователей в твоём регионе."
    )
        ...
        if data.startswith("like_"):
            ...
            async with db.execute(
                ...
                if match:
                    ...
                    await query.edit_message_text(
                        "Вы лайкнули пользователя!"
                    )

        elif data.startswith("dislike_"):

            await query.edit_message_text(
                "Вы пропустили пользователя."
            )
...
async def menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with aiosqlite.connect("bot.db") as db:
        ...
        if text == "📇 Мой профиль":
            ...
            async with db.execute(
                """
                SELECT name, bio, photo, region, interests
                FROM users
                WHERE id=?
                """,
                (user_id,)
...
async def main():
    ...
    conv_handler = ConversationHandler(
        entry_points=[
            CommandHandler("setprofile", setprofile)
        ],
